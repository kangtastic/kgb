...
extern volatile unsigned char bFrameDone ;
	...
	unsigned int data_id, unsigned int data0, unsigned int data1)
{
	...
	case DCS_LONG_WR:
	{
		...
		/* if data count is less then 4, send 3bytes data.  */
		if (data1 < 4) {
			payload = *(u8 *)(data0) |
				*(u8 *)(data0 + 1) << 8 |
				*(u8 *)(data0 + 2) << 16;

			s5p_dsim_wr_tx_data(dsim_base, payload);
		...
		/* in case that data count is more then 4 */
			...
			for (data_cnt = 0; data_cnt < data1; data_cnt+=4) {
				/*
				 * after sending 4bytes per one time, send remainder data
				 * that it is less then 4.
				 */
				if ((data1 - data_cnt) < 4) {
					if ((data1 - data_cnt) == 3) {
					...
					dev_dbg(dsim.dev, "count = 3 payload = %x, %x %x %x\n", payload,
						...
						*(u8 *)(data0 + (data_cnt + 2)));
						...
						payload = *(u8 *)(data0 + data_cnt) |
							(*(u8 *)(data0 + (data_cnt + 1))) << 8;
					dev_dbg(dsim.dev, "count = 2 payload = %x, %x %x\n", payload,
						*(u8 *)(data0 + data_cnt),
						*(u8 *)(data0 + (data_cnt + 1)));
				...
				/* send 4bytes per one time. */
					...
					dev_dbg(dsim.dev, "count = 4 payload = %x, %x %x %x %x\n", payload,
						*(u8 *)(data0 + data_cnt),
						*(u8 *)(data0 + (data_cnt + 1)),
						*(u8 *)(data0 + (data_cnt + 2)),
						*(u8 *)(data0 + (data_cnt + 3)));
	...
	unsigned char byte_clk_sel, unsigned char enable)
{
	...
	if (enable) {
		 ...
		 *
		 * in case of evt0, DSIM_TRUE is enable and DSIM_FALSE is enable for evt1.
		 */
		...
		/* in case of evt0, DSIM_FALSE is disable and DSIM_TRUE is disable for evt1. */
		if (dsim.pd->platform_rev == 1)
			s5p_dsim_enable_byte_clock(dsim_base, DSIM_TRUE);
		else
			s5p_dsim_enable_byte_clock(dsim_base, DSIM_FALSE);
...
//		s5p_dsim_sw_reset(dsim_base);
	case DSIM_STATE_INIT:
		...
		if (time_out != 0) {
//sehun_lcd			dev_info(dsim.dev, "initialization of DSI Master is successful\n");
//sehun_lcd			dev_info(dsim.dev, "DSI Master state is stop state\n");
		}
...
unsigned char s5p_dsim_set_hs_enable(unsigned int dsim_base)
{
	...
	if (dsim.state == DSIM_STATE_STOP) {
		if (dsim.e_clk_src != DSIM_EXT_CLK_BYPASS) {
			...
			//	DSIM_STATE_HSCLKEN);
			//s5p_dsim_set_data_mode(dsim_base, DSIM_TRANSFER_BYCPU,
			//	DSIM_STATE_STOP);
...
static irqreturn_t s5p_dsim_interrupt_handler(int irq, void *dev_id)
{
	...
	/*
	if (s3cfb_frame_done() && intsrc & (1 << 24))
		s3cfb_trigger();
	*/
...
extern void ams397g201_lcd_panel_init(void);
...
irqreturn_t s5p_dsim_intr(struct platform_device *pdev)
{
	...
	enable_irq(IRQ_MIPIDSI);
}

extern void tl2796_reset_lcd(void);
...
static int s5p_dsim_probe(struct platform_device *pdev)
{
	...
	if (request_irq(dsim.irq, s5p_dsim_interrupt_handler, IRQF_TRIGGER_RISING,
		    "mipi-dsi", &dsim)) {
...
#if 0//@@
	if (dsim.pd->mipi_power)
		dsim.pd->mipi_power((void *) dsim.r_mipi_1_1v,
...
//	msleep(600);
#if 0//@@
	// Register Dsim interrupt serviece routine
	request_irq(IRQ_MIPIDSI, s5p_dsim_intr, IRQF_DISABLED,  
	"dsim_general", 0);
	enable_irq(IRQ_MIPIDSI);
	s5p_dsim_set_interrupt_mask(dsim.reg_base, AllDsimIntr, 0x0);	
#endif
...
{
#if 0
	struct s5p_platform_dsim *pdata = to_dsim_plat(&pdev->dev);
#endif
	return 0;
}
