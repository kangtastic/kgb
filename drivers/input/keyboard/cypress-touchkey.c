...
static void bl_off(struct work_struct *bl_off_work) {
	if (bl_devdata == NULL || unlikely(bl_devdata->is_dead) || bl_devdata->is_powering_on || bl_on || bl_devdata->is_sleeping)
		return;
...
static int recovery_routine(struct cypress_touchkey_devdata *devdata) {
	...
	if (unlikely(devdata->is_dead)) {
		dev_err(&devdata->client->dev, "%s: Device is already dead, skipping recovery\n", __func__);
		return -ENODEV;
	}
...
{
	struct cypress_touchkey_devdata *devdata = container_of(h, struct cypress_touchkey_devdata, early_suspend);
...
static void cypress_touchkey_early_resume(struct early_suspend *h)
{
	...
	while (retry--) {
		...
		dev_err(&devdata->client->dev, "%s: touch keypad init, ret = %d\n",__func__, ret);
		if (ret == 0)
			break;
		devdata->pdata->touchkey_onoff(TOUCHKEY_OFF);
		devdata->pdata->touchkey_onoff(TOUCHKEY_ON);
		printk("%s: touch keypad force reset", __func__);
	}

	if (retry == 0) {
		dev_err(&devdata->client->dev, "%s: touch keypad not responding to commands, disabling\n", __func__);
		devdata->is_dead = true;
		up(&enable_sem);
		return;
	}
...
static struct attribute *bl_led_attributes[] = {
		&dev_attr_led.attr,
		&dev_attr_bl_timeout.attr,	// Not the best place, but creating a new device is more trouble than it's worth
		NULL
};
...
static int cypress_touchkey_probe(struct i2c_client *client, const struct i2c_device_id *id) {
...
#else
	devdata->has_legacy_keycode = data[1] >= 0xc4 || data[1] < 0x9 || (data[1] == 0x9 && data[2] < 0x9);
#endif

	if (data[1] < 0xc4 && (data[1] >= 0x8 || (data[1] == 0x8 && data[2] >= 0x9)) && devdata->has_legacy_keycode == false) {
